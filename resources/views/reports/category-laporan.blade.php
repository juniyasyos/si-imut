...
<body
    x-data='categoryReport(@json($imutBenchmarkTypes ?? []), @json($timMutuUsersData), {{ $defaultLeftSignerIndex }}, @json($rightSignerData))'
    class="bg-white text-gray-800 font-sans text-sm leading-relaxed">
        ...
        <div class="p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    <input type="radio" id="orientation-landscape" name="printOrientation" value="landscape" checked
                        @change="printOrientation = $event.target.value">
                    <label for="orientation-landscape" class="text-sm text-amber-800 cursor-pointer">Landscape</label>
                </div>
                <div class="flex items-center gap-2">
                    <input type="radio" id="orientation-portrait" name="printOrientation" value="portrait"
                        @change="printOrientation = $event.target.value">
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-wrap items-center justify-end gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ url('/siimut/daily-report-entries') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 
                       border border-gray-200 rounded-xl 
                       hover:bg-gray-200 transition">
                        Kembali
                    </a>

                    <button @click="fetchData()" class="px-5 py-2.5 text-sm font-medium text-white 
                       bg-blue-600 rounded-xl 
                       hover:bg-blue-700 transition shadow-sm">
                        Refresh
                    </button>

                    <button @click="handlePrint()" class="px-5 py-2.5 text-sm font-medium text-white 
    ...
    <!-- HEADER -->
    <x-basic-report-header title="Laporan Kategori Indikator Mutu" :additionalInfo="[
    ...
    <div x-show="loading" x-cloak class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white p-8 rounded-lg shadow-xl">
            <div class="rounded-full h-12 w-12 border-b-4 border-blue-600 mx-auto mb-4"
    ...
    <div class="bg-gray-100 border border-gray-300 rounded p-4 mb-6 text-xs">
        ...
        <div class="mt-10">
            ...
            @if($dataByImut && count($dataByImut) > 0)
                ...
                <!--        TABLE SUMMARY            -->
                <!-- =============================== -->
                <div class="mb-6 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <table class="min-w-full table-fixed border-separate border-spacing-0 text-sm">
                        ...
                        <thead>
                            <!-- Header utama -->
                            <tr class="bg-slate-800 text-[11px] font-semibold uppercase tracking-[0.08em] text-white">
                                <th class="w-14 border-b border-slate-700 px-3 py-3 text-center">
                                    No
                                </th>

                                <th class="w-80 border-b border-slate-700 px-4 py-3 text-left">
                                    Indikator Mutu
                                </th>

                                <th class="w-24 border-b border-slate-700 px-3 py-3 text-center">
                                    Target
                                </th>

                                @if($hasUnitScope)
                                    <th class="w-56 border-b border-slate-700 px-3 py-3 text-left">
                                        Unit
                                    </th>
                                @endif

                                @foreach($allMonths as $month)
                                    <th class="border-b border-l border-slate-600 bg-slate-700 px-2 py-3 text-center"
                                        colspan="3">
                                        {{ $month['label'] }}
                                    </th>
                                @endforeach
                            ...
                            <!-- Sub header -->
                            <tr class="bg-slate-50 text-[11px] font-semibold uppercase tracking-[0.06em] text-slate-500">
                                <th class="border-b border-slate-200 px-3 py-2"></th>
                                <th class="border-b border-slate-200 px-4 py-2"></th>
                                <th class="border-b border-slate-200 px-3 py-2"></th>
                                ...
                                @foreach($allMonths as $month)
                                    <th class="border-b border-l border-slate-200 px-2 py-2 text-center">N</th>
                                    <th class="border-b border-slate-200 px-2 py-2 text-center">D</th>
                                    <th class="border-b border-slate-200 px-2 py-2 text-center">%</th>
                                @endforeach
                            </tr>
                        </thead>
                        ...
                        <tbody class="text-[13px] text-slate-700">
                            @foreach($grouped as $category => $items)
                                ...
                                <tr class="bg-slate-100/80">
                                    <td colspan="{{ $colspan }}"
                                        class="border-y border-slate-200 px-4 py-3 text-xs font-semibold uppercase tracking-[0.06em] text-slate-700">
                                        {{ $category }}
                                    </td>
                                </tr>
                                ...
                                @foreach($items as $imut)
                                    ...
                                    <tr class="align-top transition-colors odd:bg-white even:bg-slate-50/40 hover:bg-slate-50">
                                        <!-- NO -->
                                        <td class="border-b border-slate-200 px-3 py-3 text-center font-medium text-slate-500">
                                            {{ $counter }}
                                        </td>

                                        <!-- INDIKATOR -->
                                        <td class="border-b border-slate-200 px-4 py-3 leading-6 text-slate-800">
                                        ...
                                        <td class="border-b border-slate-200 px-3 py-3 text-center">
                                            <span
                                                class="inline-flex min-w-[76px] items-center justify-center rounded-md border border-sky-200 bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">
                                                {{ $symbol }} {{ round($standard, 2) }}%
                                            </span>
                                        </td>

                                        @if($hasUnitScope)
                                            <td class="border-b border-slate-200 px-3 py-3 text-xs text-slate-600">
                                                @if(!empty($imut['units']))
                                                    <div class="flex flex-wrap gap-1.5">
                                                        @foreach($imut['units'] as $unit)
                                                            <span
                                                                class="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-medium text-slate-600">
                                                                {{ $unit }}
                                                            </span>
                                                        @endforeach
                                                    </div>
                                        ...
                                        @foreach($allMonths as $month)
                                            ...
                                            <td
                                                class="border-b border-l border-slate-200 px-2 py-3 text-right tabular-nums text-slate-600">
                                                {{ number_format($numerator) }}
                                            </td>

                                            <td class="border-b border-slate-200 px-2 py-3 text-right tabular-nums text-slate-600">
                                                {{ number_format($denominator) }}
                                            </td>

                                            <td class="border-b border-slate-200 px-2 py-3 text-right tabular-nums">
                                                <span
                                                    class="inline-flex min-w-[72px] justify-end rounded-md px-2 py-1 font-semibold {{ $percentClass }}">
                                                    {{ round($percent, 2) }}%
                                                </span>
                                            </td>
                                        @endforeach
                ...
                @foreach($categoryDetails as $catIndex => $cat)
                    ...
                    @else
                        ...
                        @foreach($subset as $index => $imut)
                            ...
                            <div x-data='imutNotes(@json($imut["notes"]), @json($imut["data"]))'
                                class="mt-8 rounded-2xl border border-gray-200 shadow-sm bg-white overflow-hidden imut-section">
                                ...
                                <div class="px-6 py-5 bg-gradient-to-r from-slate-800 to-slate-700 text-white">
                                    <div class="flex items-start justify-between gap-6">
                                        ...
                                        <div class="text-right">
                                            ...
                                            <div
                                                class="mt-1 inline-flex items-center gap-2 bg-white/10 border border-white/20 px-4 py-2 rounded-lg">
                                                <span class="text-sm font-semibold">
                                                    {!! $opIcon !!}
                                                    {{ $displayStd !== null ? number_format($displayStd, 2) . '%' : '-' }}{{ $note ?? '' }}
                                                </span>
                                            </div>
                                        </div>
                                ...
                                <div class="p-6">
                                        ...
                                        <div class="mb-4 bg-white border border-gray-200 rounded-lg p-4">
                                            <div class="flex flex-wrap gap-3">
                                                @foreach($typesForThis as $type)
                                                    <label
                                                        class="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer text-sm text-gray-700">
                                                        <input type="checkbox"
                                    ...
                                    <div class="grid grid-cols-2 gap-6">
                                            ...
                                            <table class="w-full text-xs border-collapse" style="table-layout: fixed;">
                                                ...
                                                <thead>
                                                    ...
                                                    <tr class="text-[10px] uppercase tracking-wide">
                                                        <th class="px-2 py-1 text-left" style="{{ $defaultBg }}; width: 120px;">Periode
                                                        </th>
                                                        @if($hasUnitScope && !empty($imut['units']))
                                                            <th class="px-2 py-1 text-left" style="{{ $defaultBg }}; width: 150px;">Unit
                                                            </th>
                                                        @endif
                                                        ...
                                                        @foreach($benchmarkCols as $col)
                                                            ...
                                                            @endphp
                                                            <th x-show="showBenchmarkCols['{{ $imut['id'] }}'] !== undefined && showBenchmarkCols['{{ $imut['id'] }}']['{{ $col['type'] }}'] === true"
                                                                x-cloak class="px-2 py-1 text-center border-l border-gray-300"
                                                                style="{{ $baseStyle }}">
                                                                Bm {{ $col['type'] }}
                                                            </th>
                                                        @endforeach
                                                        ...
                                                        <tr class="hover:bg-slate-50 transition">
                                                            ...
                                                            @if($hasUnitScope && !empty($imut['units']))
                                                                <td class="px-0.5 py-0.5 text-xs max-w-xs">
                                                                    <div class="flex flex-col gap-0.5">
                                                                        @foreach($imut['units'] as $unit)
                                                                            <span
                                                                                class="inline-flex items-center px-0.5 py-0 rounded text-[7px] font-medium bg-purple-50 text-purple-700 border border-purple-200 truncate">
                                                                                {{ $unit }}
                                                                            </span>
                                                                        @endforeach
                                                                    </div>
                                                            ...
                                                            <td class="px-3 py-1 text-center">
                                                                ...
                                                                @elseif($dataPoint['status'] === 'no-data')
                                                                    <span
                                                                        class="px-2 py-0.5 text-[10px] font-semibold rounded-full bg-slate-100 text-slate-600">
                                                                        No Data
                                                                    </span>
                                                                @else
                                                                    <span
                                                            ...
                                                            @foreach($benchmarkCols as $col)
                                                                ...
                                                                <td x-show="showBenchmarkCols['{{ $imut['id'] }}'] !== undefined && showBenchmarkCols['{{ $imut['id'] }}']['{{ $col['type'] }}'] === true"
                                                                    x-cloak class="px-3 py-1 text-center font-medium" style="{{ $tdStyle }}">
                                                    ...
                                                    @if($dataMonths > 0)
                                                        ...
                                                        <tr class="bg-slate-100 font-semibold text-slate-900">
                                                            ...
                                                            <td class="px-3 py-3 text-center">
                                                                @if($isAchieved)
                                                                    <span
                                                                        class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-emerald-200 text-emerald-800">
                                                                        Tercapai
                                                                    </span>
                                                                @else
                                                                    <span
                                                                        class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-red-200 text-red-800">
                                                                        Belum
                                                                    </span>
                                                                @endif
                                                            </td>
                                        ...
                                        <div class="flex flex-col justify-center min-h-[320px]">
                                            <canvas id="chart-{{ $imut['id'] }}" data-chart data-imut-id="{{ $imut['id'] }}"
                                                ...
                                                <div class="mb-3 space-y-2">
                                                    <template x-for="note in notes" :key="note.id">
                                                        <label class="inline-flex items-center gap-2">
                                                            <input type="checkbox" class="form-checkbox" x-model.number="selected"
                                                                :value="note.id">
                ...
                <div class="mt-10 border-t-2 border-gray-300 pt-6">
                    ...
                    <div class="flex justify-end mt-10">
                        ...
                        <div class="text-center w-56">
                            ...
                            <div class="text-sm mb-4"><br><span class="font-medium">Penanggung Jawab</span><br><span
                                    class="font-medium">Tim Mutu</span></div>
                            <div class="h-16 flex items-end justify-center mb-2">
                                <template x-if="selectedLeftSigner && selectedLeftSigner.ttd_url">
                                    <img :src="selectedLeftSigner.ttd_url" alt="Tanda Tangan"
