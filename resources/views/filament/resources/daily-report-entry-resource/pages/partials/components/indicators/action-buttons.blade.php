@if($isMonitoringMode ?? false)
    ...
    <button
        @click="console.log('🔘 [Action] Monitoring mode button clicked'); openSlideOverFast(indicator.id, selectedDate || '{{ now()->format('Y-m-d') }}')"
...
@else
    ...
    <template x-if="getActionButton(indicator.id, selectedDate).state === 'done_locked'">
        <button
            @click="console.log('🔘 [Action] Lihat Data clicked, indicator:', indicator.id, 'date:', selectedDate); openSlideOverFast(indicator.id, selectedDate || '{{ now()->format('Y-m-d') }}')"
    ...
    <template x-if="getActionButton(indicator.id, selectedDate).state === 'pending'">
        <button
            @click="console.log('🔘 [Action] Tambah Data clicked, indicator:', indicator.id, 'date:', selectedDate || '{{ now()->format('Y-m-d') }}'); openSlideOverFast(indicator.id, selectedDate || '{{ now()->format('Y-m-d') }}')"
    ...
    <!-- Default Button -->
    <template
        x-if="!getActionButton(indicator.id, selectedDate).state || getActionButton(indicator.id, selectedDate).state === 'disabled'">
        <button
            @click="console.log('🔘 [Action] Default Lihat clicked, indicator:', indicator.id, 'date:', selectedDate); openSlideOverFast(indicator.id, selectedDate || '{{ now()->format('Y-m-d') }}')"
