...
<x-filament-widgets::widget class="dark:bg-slate-900">
    <x-filament::section class="pt-6 pb-4 dark:bg-slate-900">
        ...
        @endphp
        <div class="mb-6 rounded-lg border-2 border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/30 p-5">
                ...
                <div class="flex-1">
                    ...
                    <div class="mt-3 space-y-2">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-600 dark:text-slate-400">Progres Pengisian Analisis Semua Unit</span>
                    ...
                    <div class="mt-4 space-y-2">
                        {{-- Toggle Button --}}
                        <button type="button" wire:click="toggleExpandedDetails({{ $previousReport['id'] }})" class="flex w-full items-center justify-between rounded-lg bg-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-300 dark:bg-slate-700/70 dark:text-slate-200 dark:hover:bg-slate-600">
                        ...
                        {{-- Expandable Content --}}
                        @if ($this->isDetailExpanded($previousReport['id']))
                        <div class="space-y-2 rounded-lg bg-white/30 p-3 dark:bg-slate-800/20">
                            @foreach ($stats['unit_details'] as $detail)
                            <div class="rounded-lg border border-gray-200 bg-white/80 p-3 text-xs dark:border-slate-700 dark:bg-slate-800/40">
                                {{-- Header: Unit Name + Status --}}
                                <div class="flex items-start justify-between gap-2 mb-2">
                                    <div class="flex-1">
                                        <p class="font-semibold text-gray-900 dark:text-slate-50">{{ $detail['unit_name'] }}</p>
                                        <p class="text-xs {{ $detail['is_completed'] ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-slate-400' }} font-medium mt-0.5">
                                            {{ $detail['status_text'] }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="mb-2">
                                    <div class="flex items-center justify-between text-xs mb-1">
                                        <span class="text-gray-600 dark:text-slate-400">Pengisian Analisis</span>
                                        <span class="font-semibold text-gray-900 dark:text-slate-50">{{ $detail['completed'] }}/{{ $detail['total'] }} ({{ $detail['percentage'] }}%)</span>
                                    </div>
        ...
        <div class="mb-6 rounded-lg border-2 {{ $config['border'] }} {{ $config['bg'] }} p-5">
            <div class="flex items-start justify-between gap-4">
                <div class="flex items-start gap-3 flex-1">
                    ...
                    <div class="flex-1">
                        <h4 class="font-semibold text-gray-900 dark:text-slate-50 mb-3">
                            {{ $mostUrgent['name'] }}
                        </h4>
                        ...
                        <div class="mb-4 rounded-lg {{ $config['bg'] }} border-2 {{ $config['border'] }} p-4 dark:bg-slate-800/30 dark:border-slate-700">
                            ...
                            <div class="mt-3 border-t {{ $config['border'] }} pt-3 dark:border-slate-700">
                                <p class="text-xs {{ $config['text'] }} dark:text-slate-300">
                        ...
                        <div class="mt-4 space-y-2">
                            {{-- Toggle Button --}}
                            <button type="button" wire:click="toggleExpandedDetails({{ $mostUrgent['id'] }})" class="flex w-full items-center justify-between rounded-lg bg-gray-100 px-3 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-200 dark:bg-slate-700/50 dark:text-slate-200 dark:hover:bg-slate-600">
                                <span>Detail Per Unit Kerja ({{ $stats['total_units'] }} units)</span>
                                <span class="transition-transform duration-200">
                            ...
                            {{-- Expandable Content --}}
                            @if ($this->isDetailExpanded($mostUrgent['id']))
                            <div class="space-y-2 rounded-lg bg-gray-50 p-3 dark:bg-slate-800/20">
                                @foreach ($stats['unit_details'] as $detail)
                                <div class="rounded-lg border border-gray-200 bg-white p-3 text-xs dark:border-slate-700 dark:bg-slate-800/40">
                                    {{-- Header: Unit Name + Status --}}
                                    <div class="flex items-start justify-between gap-2 mb-2">
                                        <div class="flex-1">
                                            <p class="font-semibold text-gray-900 dark:text-slate-50">{{ $detail['unit_name'] }}</p>
                                            <p class="text-xs {{ $detail['is_completed'] ? 'text-green-600 dark:text-green-400' : ($detail['is_overdue'] ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-slate-400') }} font-medium mt-0.5">
                                                {{ $detail['status_text'] }}
                                            </p>
                                        </div>
                                    ...
                                    <div class="space-y-0.5 border-t border-gray-200 pt-2 dark:border-slate-700">
                                        <div class="flex justify-between items-center">
                                            <span class="text-gray-600 dark:text-slate-400 font-semibold">📅 Deadline:</span>
                                        ...
                                        <div class="flex justify-between items-center">
                                            <span class="text-gray-600 dark:text-slate-400 font-semibold">⏱️ Sisa:</span>
                                        ...
                                        <div class="flex justify-between text-xs">
                                            <span class="text-gray-600 dark:text-slate-400">Periode:</span>
                                            <span class="text-gray-700 dark:text-slate-300">{{ $detail['period_end']->format('d M Y') }}</span>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
        ...
        @if (count($reports) > 1)
        <div class="space-y-3">
            <p class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Laporan Lainnya:</p>
            ...
            <div class="flex flex-col gap-3 rounded-lg border {{ $config['border'] }} {{ $config['bg'] }} px-4 py-3 transition hover:shadow-sm">
                <div class="flex items-center gap-4">
                    <div class="flex flex-1 items-center gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xs {{ $config['text'] }} font-semibold mt-1 dark:text-slate-300">
                ...
                <div class="space-y-2 border-t {{ $config['border'] }} pt-3 dark:border-slate-700">
                    ...
                    @if (!empty($stats['unit_details']))
                    <button type="button" wire:click="toggleExpandedDetails({{ $report['id'] }})" class="flex w-full items-center justify-between rounded-lg bg-gray-100 px-2 py-1.5 text-xs font-medium text-gray-700 transition hover:bg-gray-200 dark:bg-slate-700/50 dark:text-slate-200 dark:hover:bg-slate-600 mt-2">
                        ...
                        <div class="rounded-lg border border-gray-200 bg-white p-2.5 text-xs dark:border-slate-700 dark:bg-slate-800/40">
                            ...
                            <div class="flex items-start justify-between gap-2 mb-1.5">
                                <div class="flex-1 min-w-0">
                                    <p class="font-semibold text-gray-900 dark:text-slate-50 truncate">{{ $detail['unit_name'] }}</p>
                                    <p class="text-xs {{ $detail['is_completed'] ? 'text-green-600 dark:text-green-400' : ($detail['is_overdue'] ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-slate-400') }} font-medium mt-0.5">
                                        {{ $detail['status_text'] }}
                                    </p>
                                </div>
                            ...
                            <div class="mb-1.5">
                                <div class="flex items-center justify-between text-xs mb-0.5">
                                    <span class="text-gray-600 dark:text-slate-400">{{ $detail['completed'] }}/{{ $detail['total'] }}</span>
                                    <span class="font-semibold text-gray-900 dark:text-slate-50">{{ $detail['percentage'] }}%</span>
                                </div>
                            ...
                            @else
                            <p class="text-xs text-gray-500 dark:text-slate-400 italic mb-1.5">Tidak ada data penilaian</p>
                            @endif

                            {{-- Deadline Info (compact) --}}
                            <div class="space-y-0.5 border-t border-gray-200 pt-1.5 dark:border-slate-700 text-xs">
                                <div class="flex justify-between">
                                    <span class="text-gray-600 dark:text-slate-400">Deadline:</span>
                                    <span class="font-medium {{ $detail['is_overdue'] ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-slate-50' }}">{{ $detail['analysis_deadline']->format('d M Y') }}</span>
                                </div>
                            </div>
                        </div>
                        @endforeach
